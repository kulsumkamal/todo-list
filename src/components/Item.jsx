...
import image from './res/pink-pencil.png'

const Item = ({id, content, complete, handleCheck, handleDelete, handleEdit}) => {
    const checked = (complete) => {
        if(complete)
        return(<input className="taskCheck" type="checkbox" id={id} name="task" value={content} onChange={()=>{handleCheck(id)}} checked/>);
        return(<input className="taskCheck" type="checkbox" id={id} name="task" value={content} onChange={()=>{handleCheck(id)}}/>)
    };
    ...
    const editTask = (id) => {
        ...
        setTaskInput(<div id={"edit"+id}>
        <form onSubmit={saveEdit}>
            <input className="edit-task-input" type="text" placeholder={content} onChange={handleChange}/>
    ...
    return (
        <li>
            { checked(complete)}
            <label for="task" className="taskLabel">{content}</label>
            <span><img class="edit-task" src={image} title="Edit Task" onClick={()=> {editTask(id)}}/></span>
            {taskInput}
            <button className="deleteBtn" onClick={() => {handleDelete(id)}} title="Delete">
